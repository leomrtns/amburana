...
heap_minhash new_heap_minhash (kmerhash kmer, int sketch_size);
void del_heap_minhash (heap_minhash minh);
void heap_minhash_insert (heap_minhash minh, uint64_t hash);
double heap_minhash_calculate_distance (heap_minhash mh1, heap_minhash mh2, int idx_in_kmer_params);

bbit_minhash new_bbit_minhash (kmerhash kmer, int n_bits);
void del_bbit_minhash (bbit_minhash oph);
void bbit_minhash_insert (bbit_minhash oph, uint64_t hash);
double bbit_minhash_calculate_distance (bbit_minhash oh1, bbit_minhash oh2, int idx_in_kmer_params);
...
new_sketch_set (kmerhash kmer, int heap_mh_size, int bbit_mh_bits)
{
  ...
  sset->n_heap_mh = kmer->n_hash/3;
  sset->n_bbit_mh = kmer->n_hash - sset->n_heap_mh;
  sset->heap_mh = (heap_minhash*) biomcmc_malloc (sset->n_heap_mh * sizeof (heap_minhash));
  sset->bbit_mh = (bbit_minhash*) biomcmc_malloc (sset->n_bbit_mh * sizeof (bbit_minhash));
  for (i=0; i < sset->n_heap_mh; i++) {
    sset->heap_mh[i] = new_heap_minhash (kmer, heap_mh_size);
    sset->i_heap[i] = i;
  }
  for (i=0; i < sset->n_bbit_mh; i++) {
    sset->bbit_mh[i] = new_bbit_minhash (kmer, bbit_mh_bits);
...
del_sketch_set (sketch_set sset)
{
  int i;
  if (!sset) return;
  for (i = sset->n_heap_mh-1; i >= 0; i--) del_heap_minhash (sset->heap_mh[i]);
  for (i = sset->n_bbit_mh-1; i >= 0; i--) del_bbit_minhash (sset->bbit_mh[i]);
...
{
  int i;
  sset = new_sketch_set (kmer, heap_mh_size, bbit_mh_bits);
  link_kmerhash_to_dna_sequence (sset->kmer, dna, dna_length); 

  while ( kmerhash_iterator (kmer)) { // all magic happens here
    for (i = 0; i < sset->n_heap_mh; i++) heap_minhash_insert (sset->heap_mh[i], kmer->hash[ sset->i_heap[i] ]);
    for (i = 0; i < sset->n_bbit_mh; i++) bbit_minhash_insert (sset->bbit_mh[i], kmer->hash[ sset->i_bbit[i] ]);
  }

  for (i = 0; i < sset->n_heap_mh; i++) heap64_finalise_heap_qsort (sset->heap_mh[i]);
  return sset;
}
...
compare_sketch_set (sketch_set ss1, sketch_set ss2)
{
  ...
  if (ss1->n_heap_mh != ss2->n_heap_mh) biomcmc_error ("Sketch sets with distinct number of heap_minhash sizes");
  if (ss1->n_bbit_mh != ss2->n_bbit_mh) biomcmc_error ("Sketch sets with distinct number of bbit_minhash sizes");
  
  for (i = 0; i < ss1->n_heap_mh; i++) 
    ss1->dist[i] = ss2->dist[i] = heap_minhash_calculate_distance (ss1->heap_mh[i],ss2->heap_mh[i], sset->i_heap[i]);
  for (i = 0; i < ss1->n_bbit_mh; i++) 
    ss1->dist[i + ss1->n_heap_mh] = ss2->dist[i + ss2->n_heap_mh] = bbit_minhash_calculate_distance (ss1->bbit_mh[i],ss2->bbit_mh[i], sset->i_bbit[i]);
  return;
}

heap_minhash
new_heap_minhash (kmerhash kmer, int sketch_size)
{
  int i;
  heap_minhash minh = (heap_minhash) biomcmc_malloc (sizeof (struct heap_minhash_struct));
...
void
del_heap_minhash (heap_minhash minh)
{
  int i;
  if (!minh) return;
...
heap_minhash_insert (heap_minhash minh, uint64_t hash)
{
  heap64_insert (minh->sketch, hash);
}

void
heap_minhash_finalise (heap_minhash minh)
{
  heap64_finalise_heap_qsort (minh->sketch);
}

double
heap_minhash_calculate_distance (heap_minhash mh1, heap_minhash mh2, int idx_in_kmer_params)
{
...
bbit_minhash
new_bbit_minhash (kmerhash kmer, int n_bits)
{
  int i, j;
  bbit_minhash oph = (bbit_minhash) biomcmc_malloc (sizeof (struct bbit_minhash_struct));
  if (n_bits < 4)  n_bits = 4; // good value 8~10 
...
void
del_bbit_minhash (bbit_minhash oph)
{
  int i;
  if (!oph) return;
...
void
bbit_minhash_insert (bbit_minhash oph, uint64_t hash)
{
...
double
bbit_minhash_calculate_distance (bbit_minhash oh1, bbit_minhash oh2, int idx_in_kmer_params)
{
...
new_heap_minhash_from_dna (char *dna, size_t dna_length, int sketch_size, bool dense)
{
  ...
  heap_minhash minh = new_heap_minhash (sketch_size, kmer->n_hash); // use all hashes available by kmerhash
  minh->kmer = kmer;
...
new_bbit_minhash_from_dna (char *dna, size_t dna_length, int n_bits, bool dense)
{
  ...
  kmerhash kmer = new_kmerhash (dense); // false = 4 bits per site (o.w. 2 bits) 
  link_kmerhash_to_dna_sequence (kmer, dna, dna_length); // not ideal, since we can create ONE kmerhash for whole analysis 
  bbit_minhash oph = new_bbit_minhash (n_bits, kmer->n_hash);
  oph->kmer = kmer; 
